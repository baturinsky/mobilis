...
function drawSquareTile(ctx: CanvasRenderingContext2D, tile: CanvasImageSource[], pos: number, columns: number, connect?: (pos: number) => boolean) {
    ...
    for (let corner = 0; corner < 4; corner++) {
        let dx = corner % 2 ? 1 : -1;
        let dy = corner > 1 ? 1 : -1;
        let xNear = !connect(pos + dx) ? 1 : 0;
        let yNear = !connect(pos + dy * columns) ? 1 : 0;
        let kind = 0;
        if (xNear || yNear) {
            kind = xNear + yNear * 2;
...
function drawTile(
    ctx: CanvasRenderingContext2D, tile, pos: number, layout: number, columns: number,
    neighborDeltas?: number[][], connect?: (pos: number) => boolean
) {
    if (!tile) return;
...
function createSprites(tileset, layout) {
    ...
    let sprites: (HTMLCanvasElement[]|HTMLCanvasElement)[] = [];
